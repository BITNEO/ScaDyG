...
class Encoder(nn.Module):
    ...
    def forward(self, src_emb,src_emb_2,mode = 'train',step1 = 0,step2=0):# time feature, node feature  src_emb:[node_num,seq_len,node_feat_dim]
        ...
        if mode == "val" and step2==0:
            torch.save(new_weight, './weights/node_tensor_file'+str(step1)+'.pth')
        torch.save(self.weight, './weights/node_self_weight.pth')
...
def train_scalable_tgn(model, model_transformer,optimizer, device, graph_l,node_label, logger, train_n,val_n,test_n,args):
    ...
    with torch.no_grad():
        for idx in range(len(graph_test)):
            ...
            hist_node_feature_list = hist_node_feature_list[pre_nodes_id] 
            hist_node_feature_list = hist_node_feature_list.to(device)  
            hist_node_feature_list = model_transformer(hist_node_feature_list,hist_node_feature_list,mode = 'val',step1 = idx)
