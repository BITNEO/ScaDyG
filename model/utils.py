...
def create_optimizer_1(opt, model, lr, weight_decay, get_num_layer=None, get_layer_scale=None):
    opt_lower = opt.lower()
...
def gen_negative_edges(edge_index, num_neg_per_node, num_nodes):
    ...
    num_neg_per_node = int(20 * num_neg_per_node)  # add some redundancy.
...
@torch.no_grad()
def report_rank_based_eval_meta(model, graph, x, next_graph,fast_weights, num_neg_per_node: int = 1000):
    if num_neg_per_node == -1:
        # Do not report rank-based metrics, used in debug mode.
        return 0, 0, 0, 0
    # Get positive edge indices.
    edge_index = next_graph.edge_label_index[:, next_graph.edge_label == 1]
    ...
    next_graph.edge_label_index = new_edge_label_index.to('cpu').long()
    next_graph.edge_label = new_edge_label.to('cpu').long()
    


    # move state to gpu
    pred = model(graph, x,next_graph.edge_label_index, fast_weights)
    pred = pred.to('cpu')

    mrr, recall_at = fast_batch_mrr_and_recall(next_graph.edge_label_index, next_graph.edge_label,
                                               pred, num_neg_per_node, graph.num_nodes())

    return mrr, recall_at[1], recall_at[3], recall_at[10]

@torch.no_grad()
def report_rank_based_eval_scalable(model, graph, x,  num_neg_per_node: int = 1000):
    if num_neg_per_node == -1:
        # Do not report rank-based metrics, used in debug mode.
        return 0, 0, 0, 0
    # Get positive edge indices.
    ...
    # Construct evaluation samples.
    tmp_index = graph.edge_label_index
    tmp_label = graph.edge_label
    ...
    # move state to gpu
    pred = model(graph, x,mode='val')
    pred = pred.detach().to('cpu').squeeze(1)

    mrr, recall_at = fast_batch_mrr_and_recall(graph.edge_label_index, graph.edge_label,
                                               pred, num_neg_per_node, graph.num_nodes())
    graph.edge_label_index = tmp_index
    graph.edge_label = tmp_label

    return mrr, recall_at[1], recall_at[3], recall_at[10]

@torch.no_grad()
def report_rank_based_eval_minibatch(model, graph, x,  num_neg_per_node: int = 1000):
    if num_neg_per_node == -1:
        # Do not report rank-based metrics, used in debug mode.
        return 0, 0, 0, 0
    # Get positive edge indices.
    ...
    # move state to gpu
    pred = model(graph.edge_label_index , x,mode='rank')
    pred = pred.to('cpu').squeeze(1)
