...
#     """
#     For each positive edge, generate a negative edge. The source of the negative edge is the same as the positive edge, and the target is randomly generated, ensuring that the negative edge does not duplicate any existing edges.
#
#     :param edges: The input list of edges, where each element is a pair [u, v]
#     :param max_node_id: The maximum ID value of all nodes in the graph
#     :return: The list of generated negative edges, where each element is a pair [u, v]
#     """
#     # Convert the list of positive edges to a set for easy subsequent duplicate checks
...
#     for u, _ in edges:
#         while True:
#             # Randomly generate a new target v
#             v_new = random.randint(0, max_node_id)
#             # Check if the generated negative edge duplicates any existing edges
#             if (u, v_new) not in edge_set:
#                 negative_edges.append([u, v_new])
#                 break  # Exit the loop after successfully generating a negative edge
...
def negative_sampling(edges, max_node_id):
    ...
    :return: The list of generated negative edges, where each element is a pair [u, v]
    """
    # Convert the list of positive edges into a set form for easy subsequent duplicate checks.
    edges = edges.t().tolist()
    edge_set = set(tuple(edge) for edge in edges)
    negative_edges = []
    max_node_id = max(max(u, v) for u, v in edges)
    ...
    for u, _ in edges:
        while True:
            # Randomly generate a new target vertex v.
            u_new = random.randint(0, max_node_id)
            v_new = random.randint(0, max_node_id)
            # Check if the generated negative edge duplicates any existing edges.
            if (u_new, v_new) not in edge_set:
                negative_edges.append([u_new, v_new])
                break  # Jump out of the loop after successfully generating a negative edge.
    all_edges = edges + negative_edges
    labels = [1] * len(edges) + [0] * len(negative_edges)
